...
def monitor_packets_live(switch, port):
		...
		try:
				while True:
						...
						try:
								# читаем данные
								data = switch.session.read_nonblocking(size=10000, timeout=1).decode(errors="ignore")
								...
								print(f"\033[2K\rRX: {rx_bytes} bytes ({rx_mbps} Mbs) | TX: {tx_bytes} bytes ({tx_mbps} Mbs)", end="", flush=True)
